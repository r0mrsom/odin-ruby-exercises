...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs an incorrect value once, then a valid input' do
      before do
        invalid_input = 'a'
        valid_input_next = '3'
        allow(game_input).to receive(:gets).and_return(invalid_input, valid_input_next)
      end

      it 'completes loop and displays error message once' do
        min = game_input.instance_variable_get(:@minimum)
        max = game_input.instance_variable_get(:@maximum)
        message = "Input error! Please enter a number between #{min} or #{max}."
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        invalid_input2 = '11'
        valid_input_next = '3'
        allow(game_input).to receive(:gets).and_return(invalid_input, invalid_input2, valid_input_next)
      end
  ...
  describe '#verify_input' do
    ...
    context 'when given a valid input as argument' do
      it 'returns valid input' do
        ...
        expect(game_input2.verify_input(min, max, 5)).to eql(5)
      end
    end
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        min = game_input2.instance_variable_get(:@minimum)
        max = game_input2.instance_variable_get(:@maximum)
        expect(game_input2.verify_input(min, max, 100)).to be_nil
      end
    end
  end
  ...
  describe '#display_turn_order' do
    ...
    subject(:something_to_test) { described_class.new(0, 10) }
    let(:new_binary) { instance_double(BinarySearch) }

    before do
      # You'll need to create a few method stubs.
      allow(new_binary).to receive(:make_guess)
      allow(something_to_test).to receive(:display_guess)
      allow(new_binary).to receive(:update_range)
    end

    # Command Method -> Test the change in the observable state
    it 'increases guess_count by one' do
      expect { something_to_test.display_turn_order(new_binary) }.to change {
        something_to_test.instance_variable_get(:@guess_count)
      }.from(0).to(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(new_binary).to receive(:make_guess).and_return(5)
      something_to_test.display_turn_order(new_binary)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends update_range' do
      expect(new_binary).to receive(:update_range)
      something_to_test.display_turn_order(new_binary)
    end
