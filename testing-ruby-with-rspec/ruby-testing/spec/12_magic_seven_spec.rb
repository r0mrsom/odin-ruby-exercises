...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    subject(:something) { described_class.new(3) }
    it 'should return 7' do
      previous_step = 10
      result = something.subtract_random_number(previous_step)
  ...
  describe '#play' do
    subject(:something1) { described_class.new(3) }
    context 'when the random number is ...' do
      # remove the 'x' before running this test
      it 'will return 7' do
        expect(something1.play).to eq(7)
      end
    end

    context 'when the random number is ...' do
      subject(:something2) { described_class.new(-500) }
      # remove the 'x' before running this test
      it 'will return 7 even at negative numbers' do
        expect(something1.play).to eq(7)
      end
    end

    context 'when the random number is ...' do
      subject(:something3) { described_class.new(2_293_819) }
      # remove the 'x' before running this test
      it 'will return 7 even at huge numbers' do
        expect(something1.play).to eq(7)
      end
    end
  end
