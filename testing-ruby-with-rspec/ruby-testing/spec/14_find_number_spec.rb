...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 5 and max is 9' do
      subject(:game_guessing2) { described_class.new(5, 9, random_number).make_guess }
      it 'returns 7' do
        expect(game_guessing2).to eql(7)
      end
    end

    context 'when min is 8 and max is 9' do
      subject(:game_guessing2) { described_class.new(8, 9, random_number).make_guess }
      it 'returns 8' do
        expect(game_guessing2).to eql(8)
      end
    end

    context 'when min is 0 and max is 3' do
      subject(:game_guessing2) { described_class.new(0, 3, random_number).make_guess }
      it 'returns 1' do
        expect(game_guessing2).to eql(1)
      end
    end

    context 'when min and max both equal 3' do
      subject(:game_guessing2) { described_class.new(3, 3, random_number).make_guess }
      it 'returns 3' do
        expect(game_guessing2).to eql(3)
      end
    end
  end
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      let(:random_number1) { double('random_number') }
      subject(:game) { described_class.new(0, 9, random_number1, 5) }
      ...
      it 'is game over' do
        allow(random_number1).to receive(:value).and_return(5)
    ...
    context 'when guess and random_number are not equal' do
      let(:random_number2) { double('random_number') }
      subject(:game) { described_class.new(0, 9, random_number2, 5) }
      it 'is not game over' do
        allow(random_number2).to receive(:value).and_return(4)
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:other_guess) { described_class.new(5, 8, number_range, 7) }
      it 'updates min to the same value as max' do
        expect { other_guess.update_range }.to change { other_guess.min }.from(5).to(8)
      end

      it 'does not update max' do
        expect { other_guess.update_range }.not_to(change { other_guess.max })
      end
    end
  end
end
