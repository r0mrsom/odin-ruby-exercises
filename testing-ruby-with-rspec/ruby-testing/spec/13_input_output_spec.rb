...
describe NumberGame do
  ...
  describe '#initialize' do
    context 'when solution is initialized' do
      ...
      matcher :be_between_zero_and_nine do
        match { |result| result.between?(0, 9) }
      end
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:wrong_guess) { described_class.new(3, '5') }
      # remove the 'x' before running this test
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        input2 = 'b'
        valid_input = '5'
        allow(game_loop).to receive(:player_input).and_return(input1, input2, valid_input)
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 4 and over' do
      subject(:more_than_four) { described_class.new(5, '5', 6) }
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        congrats_phase = "That was hard. It took you 6 guesses!\n"
        expect { more_than_four.final_message }.to output(congrats_phase).to_stdout
      end
    end
  end
end
